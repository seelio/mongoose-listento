...
module.exports = function listenToPlugin(schema, mongoose) {
  ...
  var _getModel = function _getModel(model, callback) {
    if ('string' === typeof model) {
      if (mongoose.modelSchemas[model]) {
        callback(mongoose.model(model));
      } else {
        process.nextTick(function() {
          _getModel(model, callback)
        });
      }
    } else {
      callback(model);
  ...
  schema.listenTo = function listenTo(model, event, listener) {
    _getModel(model, function(model) {
